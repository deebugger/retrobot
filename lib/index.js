...
const numMessages = process.env.NUM_MESSAGES || 5;
let inRetro = false;
let guid = 0;
...
function command(message) {
  ...
  if (aliases[cmd] === summary) {
    const top = retrospective.needswork.filter(_ => !!_.plusone).sort((a, b) => {
      return a.plusone < b.plusone ? 1 : -1;
    }).slice(0, numMessages).map(r => {
      return ':+1:'.repeat(r.plusone) + ' ' + r.text.replace(/^\d+\.\s/, '');
    });

    if (top.length === 0) {
      return reply(message, ':male-detective: I\'ve looked everywhere, but no one voted on any of the `needs work` items with a :+1: - please vote and run `summary` again');
    }
